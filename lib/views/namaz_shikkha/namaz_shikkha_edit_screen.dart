...
class NamazShikkhaEditScreen extends StatefulWidget {
  //const NamazShikkhaEditScreen({Key? key}) : super(key: key);
  String? title;
  String? details;
  String? docId;
  NamazShikkhaEditScreen(
      {required this.title, required this.details, required this.docId});

  @override
  State<NamazShikkhaEditScreen> createState() => _NamazShikkhaEditScreenState();
}

class _NamazShikkhaEditScreenState extends State<NamazShikkhaEditScreen> {
  String? newTitle, newDetails;
  TextEditingController? newTitleController, newDetailsController;
  ...
  void initState() {
    ...
    newTitleController = TextEditingController(text: newTitle);
    newDetailsController = TextEditingController(text: newDetails);
  }
          ...
          IconButton(
            onPressed: () {
              final dataUpdate = FirebaseFirestore.instance
                  .collection('namazShikkha')
                  .doc(widget.docId.toString());
              dataUpdate.update({'title': newTitle, 'data': newDetails});
              Navigator.pop(context);
            },
