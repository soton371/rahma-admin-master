...
import 'package:rahma_admin/views/dua/dua_add_screen.dart';
import 'package:rahma_admin/views/dua/dua_edit_screen.dart';
  ...
  Widget build(BuildContext context) {
    return status? Scaffold(
      appBar: AppBar(
        ...
        title: Text("দুআ সমূহ",style: TextStyle(
            color: MyColors.text
        )),
        centerTitle: true,
        actions: [
          IconButton(
              onPressed: ()=>Navigator.push(context, MaterialPageRoute(builder: (_)=>const DuaAddScreen())),
              icon: Icon(Icons.add, color: MyColors.primary,)
          )
        ],
      ),
      body: StreamBuilder(
          stream: FirebaseFirestore.instance.collection("Dua's").snapshots(),
              ...
              return ListView(
                  ...
                  children: snapshot.data!.docs.map((document){
                    return GestureDetector(
                        onTap: (){
                          Navigator.push(context, MaterialPageRoute(builder: (context) => DuaEditScreen(arabic: document['arabic'], docId: document.id,title: document['title'],bengali: document['bengali'],pronounce: document['pronounce'],)));
                        },
                        child: Card(
